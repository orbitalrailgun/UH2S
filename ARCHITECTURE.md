...
| `storage_files` | id, owner, path, name, format, size_bytes, rows, columns, created_ts, updated_ts, ttl | **файловые** записи хранилища: данные лежат файлом на диске (`app/storage_files.py`, каталог `UH2S_STORAGE_DIR`), в БД только метаданные. Пишутся загрузкой в разделе «Хранилище» или из DSL — `SAVE(<таблица>, storage_file[, ttl]) AS <ключ>` (parquet, иначе csv). Читает `duckdb_im` напрямую (авторегистрация VIEW по имени ключа); `LOAD`/`sqlite3_im` их не материализуют |
...
| `jira_sm` | search_issues (JQL), get_issue (коллекции → *_count), get_issue_changelog, get_issue_comments, get_issue_worklogs, get_issue_attachments, get_issue_issuelinks, search_cmdb (Assets/Insight AQL/IQL), search_cmdb_freetext (Insight `/rest/insight-am/1/search`, `criteriaType=FREETEXT`), get_cmdb_object (один объект по id или ключу, тот же shape, что у search_cmdb), get_cmdb_history (audit log объекта: `/rest/insight-am/1/assets/{key}/audits`, фильтры criteria/since/until); повторы транзиентных неудач (сеть/таймаут/429/5xx, с уважением `Retry-After`) — `max_retries`/`retry_backoff_seconds`/`retry_on_status` — Jira REST v2 (on-prem/DC), auth bearer/basic; issues разворачиваются в плоские поля; CMDB-объекты — в таблицу с колонками по **именам атрибутов** (`shape=table`, см. `app/sources/additional/cmdb.py`) | реализован |
