...
| `jira_sm` | search_issues (JQL), get_issue (по ID; comment→comment_count), get_issue_changelog, get_issue_comments, search_cmdb (Assets/Insight AQL) — Jira REST v2 (on-prem/DC), auth bearer/basic; issues разворачиваются в плоские поля | реализован |
