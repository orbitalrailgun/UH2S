...
- `jira_sm:search_issues(jql, [limit], [fields], [expand], [raw])`, `jira_sm:get_issue(issue_id, [expand], [raw])`, `jira_sm:get_issue_changelog(issue_id, [raw])`, `jira_sm:get_issue_comments(issue_id, [limit], [raw])`, `jira_sm:search_cmdb(aql, [limit], [cmdb_path], [flatten])` — заявки разворачиваются в плоские поля (`fields` поднимаются наверх; `comment` сводится к `comment_count`); `raw=true` — исходный JSON
