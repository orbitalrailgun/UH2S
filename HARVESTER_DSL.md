...
свёрнут). Это поиск-каталог с кнопкой «Вставить», объединяющий:
- **команды DSL** — синтаксис и шаблоны (`DEF`/`CALC`/`GET`/`APPLY`/`PRINT`/`SHOW`/`SAVE`/`LOAD`/`NOTIFY`);
- **функции источников** — живые сигнатуры из реестра (`GET <source>:<func>(...)` с обязательными
  параметрами и примерами значений), включая `llm`, `elastic_requests`, `duckdb_im` и др.;
- **сохранённые скрипты** — доступные по ролям, для `GET script:<имя>(...)`;
- **заметки базы знаний** — вставляются как комментарий `/* ... */`.

Поиск фильтрует по названию/сигнатуре/тексту; «Вставить» добавляет сниппет в скрипт (новые команды
соединяются через `|`). Каталог функций и заметок строится динамически — новые источники и заметки
