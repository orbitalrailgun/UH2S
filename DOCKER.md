...
| `UH2S_STORAGE_DIR` | каталог **файловых** записей хранилища (деф. `<корень>/storage_files`) — там лежат сами данные | — |
| `UH2S_UPLOAD_MAX_BYTES` | предел размера загружаемого файла (деф. 16 ГиБ) | — |
| `UH2S_STORAGE_FILE_THRESHOLD_BYTES` | от какого размера таблица хранится файлом, а не строками в БД (деф. 64 МиБ) | — |
...
**Файловые записи хранилища.** Крупные таблицы (от `UH2S_STORAGE_FILE_THRESHOLD_BYTES`) кладутся файлом
в `UH2S_STORAGE_DIR`, в БД остаются только метаданные, а `duckdb_im` читает файл напрямую. Этот каталог
**обязательно смонтируйте томом**: иначе при пересоздании контейнера файлы исчезнут, записи в БД
останутся без данных (раздел «Хранилище» подчистит их как сироты при следующем открытии). Места нужно с
запасом: при загрузке файл транзиентно занимает ~2–3 своих размера (спул starlette + temp NiceGUI +
итоговая копия).
