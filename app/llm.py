...
def build_agent_system_prompt(objects_lines, history_lines, sources_catalog=None):
    ...
    parts = [
        ...
        "пользователя и пришлёт результат (статусы шагов, число строк, колонки) сообщением "
        "«РЕЗУЛЬТАТ ВЫПОЛНЕНИЯ СКРИПТА». Анализируй результат и при необходимости исправляй и снова ```run. "
        "За один ответ — не более ОДНОГО блока ```run. Когда скрипт рабочий — приведи финальную версию "
        "в блоке ```harvester ...``` с кратким пояснением и НЕ выводи ```run. Если запуск не нужен — "
