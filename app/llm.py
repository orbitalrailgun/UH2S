...
def build_agent_system_prompt(memory_context=None):
    ...
    sections = [
        ...
        "- `GET <llm_объект>:line_analysis(data=\"tbl\", instructions=\"...\", [knowledge_base=true], [temp_notes=true]) AS out` — "
        ...
        "`temp_notes=true` включает последовательный проход с эфемерными заметками прогона (поле `_note` видно "
        "на следующих строках) — полезно для кластеризации/корреляции связанных строк.\n"
