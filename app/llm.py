...
def build_agent_system_prompt(objects_lines, history_lines, sources_catalog=None):
    ...
    parts = [
        ...
        "DSL-скрипты и объекты. Отвечай на русском, кратко и по делу. При написании GET учитывай "
        "обязательные параметры функций источников (см. каталог ниже).",
        "# Протокол выполнения\n"
        "Чтобы ПРОВЕРИТЬ скрипт, выведи его в блоке ```run ...``` — система выполнит его от имени "
        "пользователя и пришлёт результат сообщением «РЕЗУЛЬТАТ ВЫПОЛНЕНИЯ СКРИПТА»: статусы шагов, а также "
        "ВСЕ переменные и таблицы скрипта (как list of dict). Тебе НЕ нужны PRINT/SHOW/SAVE, чтобы увидеть "
        "данные — они приходят автоматически. В блоке ```run НЕ используй SHOW и SAVE (это вывод только для "
        "человека; на этапе подготовки они игнорируются). Анализируй результат, при необходимости исправляй "
        "и снова ```run (не более ОДНОГО блока ```run за ответ). Когда скрипт рабочий — приведи финальную "
        "версию в блоке ```harvester ...``` (в ней уже можно использовать PRINT/SHOW/SAVE для пользователя), "
        "с кратким пояснением и БЕЗ ```run. Если запуск не нужен — просто ответь текстом.",
        "# Язык скриптов (DSL)\n" + PROJECT_DOC,
    ]
    if sources_catalog:
