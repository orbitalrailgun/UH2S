...
def build_agent_system_prompt(memory_context=None):
    ...
    sections = [
        ...
        "   Отдельно: объект типа `llm` можно использовать КАК ИСТОЧНИК для анализа/обогащения данных — "
        "`GET <llm_объект>:line_analysis(data=\"tbl\", instructions=\"...\", [knowledge_base=true])` добавляет к "
        "каждой строке сгенерированные поля; `data_analysis(...)` анализирует весь набор в `[{}]`.\n"
